...
describe RealWorld::Article::Commands::UnfavoriteArticle do
  ...
  context 'when the article exists' do
    ...
    context 'when the user has not favorited the article yet' do
      it { is_expected.to fail_with_errors(:article_not_favorited) }
    end

    context 'when the user has favorited the article already' do
      before { repository.favorite(some_article.merge(favorites_count: 1), user_id) }
